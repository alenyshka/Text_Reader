...
VOID print_glyphs(HWND hWnd)
{
	...
	int summWidth = 0, height = 0, maxHeight = 0; 
	count_string = 0; 
	...
	for (itemList = ++glyph_list.begin(); itemList != glyph_list.end(); itemList++)
	{
		...
		symbol->draw(hDC, summWidth, CORRECT_CARET_POSITION + height, font);
...
void key_up(HWND hWnd)
{
	if (heightPositionCarret != 0)
	{
		...
		//make carretposition on temp_count_string-1
		int count_string = 0;
